...
pub fn fb_partition(gw: Word, awb: &WBank) -> FbMap<WBank> {
  ...
  for aw in &awb.data {
    ...
    let wb2: &mut WBank = map.entry(fb).or_insert_with(WBank::new);
    wb2.data.push(*aw);
  }
  map
}
...
pub fn top_words(gwb: &WBank, awb: &WBank, hd: &HData, n: usize) -> Vec<Word> {
  let mut tups: Vec<(Word, f64)> = gwb
    .data
    .iter()
    .map(|gw| (*gw, heuristic(*gw, awb, hd)))
    .collect();
  tups.sort_by(|(_, f1), (_, f2)| f1.partial_cmp(f2).unwrap());
  tups.iter().map(|(gw, _)| *gw).take(n).collect()
}
